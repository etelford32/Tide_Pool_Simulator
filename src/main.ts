...
async function init() {
  try {
    console.log('[INIT] Starting tide pool simulator v0.1.1 (compat build)');

    // Initialize Rapier WASM (compat build has proper init function)
    console.log('[RAPIER] Initializing Rapier compat build...');
    await RAPIER.init();
